...
def load_to_sql(file_path):
    ...
    engine = sqlalchemy.create_engine(f"postgresql+psycopg2://{conn.login}:{conn.password}@titanic-survival-prediction_f9b101-postgres-1:{conn.port}/{conn.schema}")
...
with DAG(
    ...
    list_files = GCSListObjectsOperator(
        task_id="list_files",
        bucket="titanic-survival-predict-mlops",
    )
