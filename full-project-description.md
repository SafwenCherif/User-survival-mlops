...
**Course context:** Udemy — *Beginner to Advanced MLOps on GCP* — Project 3: *User Survival Prediction with Astro Airflow, SQL, Redis, Grafana & Prometheus*
...
| `Materials/` | Instructor course zip; not part of *your* project |
| `logs/` | Runtime noise |
...
**Why:** Airflow must authenticate to GCS without interactive user login. Least surprise for the course is a dedicated SA + keyfile path inside the container.
...
**Why:** Local Airflow that matches course materials; Google provider enables GCS operators.
...
| Scope `cloud-platform` | Broad Google API access for course simplicity |
...
| `start_http_server(8000)` | Extra metrics server (course pattern); scraping uses `:5000/metrics` |
...
| Topic | Windows (instructor) | Ubuntu (this project) |
|-------|----------------------|------------------------|
| Astro install | winget / exe | `curl install.astronomer.io` or user-local GitHub binary in `~/.local/bin` |
...
Treat this as a **learning stack**, not a hardened production deployment (Owner role on SA is course-convenient but overly privileged for real systems).
